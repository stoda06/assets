...
def run_powershell_command(request, script):
#    powershell_script = "Get-Process"  # Replace with your actual PowerShell script
    output = subprocess.check_output(['pwsh', '-ExecutionPolicy', 'Bypass', '-Command', powershell_script], text=True)
    return HttpResponse(f"PowerShell Output: {output}")
...
def laptop_configuration(request):
    if request.method == "POST":
        ...
        input_string1 = run_powershell_command(request, string)
        input_string = input_string1.stdout
        print(197199188917625)
