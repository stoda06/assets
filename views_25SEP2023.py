...
#class ObtainAuthToken(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if user:
            print('i am from ObtainAuthToken my name is ',user)
            token, created = Token.objects.get_or_create(user=user)
            print('i am from ObtainAuthToken my token is ',token)
...
class YourApiView(APIView):
    ...
    def get(self, request):
        # Your logic here
        return Response({"message": "Your data"}, status=status.HTTP_200_OK)
...
def add_system_info(request):
    # Extract the system information from the request data
    data = request.data
    try:
        # Create a new SystemInfo object and save it to the database
        SystemInfo.objects.create(
            ...
            disk_drive_model=data.get('disk_drive_model'),
            disk_drive_size=data.get('disk_drive_size'),
            network_adapter_description=data.get('network_adapter_description'),
   ...
   #     system_info = SystemInfo.objects.create(
    #        manufacturer=data.get('manufacturer'),
     #       model=data.get('model'),
      #      processor=data.get('processor'),
       #     serial_number=data.get('serial_number'),
        #    memory=float(data.get('memory')),
         #   disk_drive_model=data.get('disk_drive_model'),
          #  disk_drive_size=float(data.get('disk_drive_size')),
           # network_adapter_description=data.get('network_adapter_description'),
           # ip_address=data.get('ip_address'),
           # computer_name=data.get('computer_name'),
       # )
...
def laptop_configuration(request):
    if request.method == "POST":
        ...
        print('path of the script file',script_path)
...
#        input_string  subprocess.check_output(['pwsh', '-ExecutionPolicy', 'Bypass', '-Command', powershell_script], text=True)
        #input_string1 = subprocess.run(['snap', 'run', 'pwsh', '-File', script_path], capture_output=True, text=True)
        input_string1 = subprocess.run(['pwsh', '-File', script_path], capture_output=True, text=True, encoding='utf-8')
 
        #input_string1 = subprocess.run(['pwsh', '-File', script_path], capture_output=True, text=True)
        input_string = input_string1.stdout
        print('#######')
        print('its input_string')
        print(input_string)
        error = input_string1.stderr
        print('its Error')
        print('input_string1 Error',error)
...
#        input_string = subprocess.check_output(['powershell', '-File', script_path], shell=True,text=True)
        ...
        print(serial_no)
#        if any(var is None for var in [disk_drive_matches, network_adapter_matches, serial_no]):
#            return JsonResponse({'error': 'One or more essential variables are None'})
